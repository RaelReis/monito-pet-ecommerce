...
const VARIANT_STYLE = {
  normal:"py-3 px-7 text-primary-base ring ring-primary-base rounded-full font-medium flex justify-center items-center gap-2",
...
export function Button({ variant='normal', children, className, ...props }: ButtonProps) {
  return (
    <button className={`${VARIANT_STYLE[variant]} ${className}`} {...props}>
      {children}
    </button>
  );
}
